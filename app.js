...
function draw(drawLast) {
	...
	for (x = drawLast ? image.length - 1 : 0; x < image.length; x++) {
		...
		switch (shape.tool) {
			case 'drawPixel':
				ctx.fillRect(shape.xs * gw, shape.ys * gh, shape.w * gw, shape.h * gh)
				break
			...
			case 'drawRect':
				...
				for (xa = 1; xa < shape.h - 1; xa++) {
					...
					ctx.fillRect(shape.xs * gw + shape.w * gw - gw, shape.ys * gh + xa * gh, gw, gh)
				}
				ctx.fillRect(shape.xs * gw, shape.ys * gh + shape.h * gh - gh, shape.w * gw, gh)
				break
			case 'fillRect':
				ctx.fillRect(shape.xs * gw, shape.ys * gh, shape.w * gw, shape.h * gh)
				break
			...
			case 'drawLine':
				...
				for (i = 0; i < Math.max(Math.abs(shape.w), Math.abs(shape.h)); i++) {
					ctx.fillRect(
						shape.xs * gw +
							Math.round(i * (Math.abs(shape.w) > Math.abs(shape.h) ? 1 : 1 / a)) *
								gw *
								Math.sign(Math.abs(shape.w) > Math.abs(shape.h) ? shape.w : shape.h),
						shape.ys * gh +
							Math.round(i * (Math.abs(shape.w) > Math.abs(shape.h) ? a : 1)) *
								gh *
								Math.sign(Math.abs(shape.w) > Math.abs(shape.h) ? shape.w : shape.h),
						gh,
						gw
			...
			case 'drawCircle':
				...
				for (i = -r; i < r * 2; i++) {
					ctx.fillRect(shape.xs * gw - i * gw, shape.ys * gh - Math.round(Math.sqrt(r ** 2 - i ** 2)) * gh, gw, gh)
					ctx.fillRect(shape.xs * gw + i * gw, shape.ys * gh + Math.round(Math.sqrt(r ** 2 - i ** 2)) * gh, gw, gh)
				}
				for (i = -r; i < r * 2; i++) {
					ctx.fillRect(shape.xs * gw - Math.round(Math.sqrt(r ** 2 - i ** 2)) * gw, shape.ys * gh - i * gh, gw, gh)
					ctx.fillRect(shape.xs * gw + Math.round(Math.sqrt(r ** 2 - i ** 2)) * gw, shape.ys * gh + i * gh, gw, gh)
				}
				break
			case 'fillCircle':
				...
				if (e.keyCode >= 32 && e.keyCode <= 255) {
					currentText.letters.push({
						text: font.filter((ele, i, arr) => {
							if (i >= e.key.charCodeAt() * 5 && i <= e.key.charCodeAt() * 5 + 4) {
								return arr[i]
							}
						}),
						letter: e.key,
...
document.addEventListener('mousedown', (e) => {
	...
	if (x > sx && x < sx + c.getBoundingClientRect().width + sx && y > sy && y < gh * ph + sy) {
		if (tool != 'erase') {
			if (tool == 'text') {
				image.push({
					tool: 'text',
	...
	function displayPaint(e) {
		...
		if (x > sx && x < sx + c.getBoundingClientRect().width + sx && y > sy && y < gh * ph + sy) {
			if (image[image.length - 1].tool == 'drawLine' || image[image.length - 1].tool == 'drawCircle') {
				...
				while (i < image.length - 1) {
					...
					if (e.tool == 'drawRect' || e.tool == 'drawCircle') {
						if ((xt == e.xs || xt < e.xs + e.w) && (yt > e.ys || yt < e.ys + e.width)) {
							image.splice(i, 1)
							console.log(image)
						}
					} else if (xt > e.xs && xt < e.xs + e.w && yt > e.ys && yt < e.ys + e.width) {
						image.splice(i, 1)
					}
					i++
				}
...
function build() {
	...
	for (i = 0; i < image.length; i++) {
		...
		switch (image[i].tool) {
			...
			case 'drawRect':
				outputString +=
					image[i].tool + '(' + image[i].xs + ', ' + image[i].ys + ', ' + image[i].w + ', ' + image[i].h + ', ' + '1);'
				break
			case 'text':
				...
				break
			case 'drawPixel':
				outputString += image[i].tool + '(' + image[i].xs + ', ' + image[i].ys + ');'
				break
			case 'drawLine':
			case 'drawCircle':
				outputString +=
